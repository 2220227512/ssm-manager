	...
	<div class="pagination">
		...
		第  ${pageBean.page }/${pageBean.totalPage }页
			<c:if test="${pageBean.page != 1 }">
				<a href="${ pageContext.request.contextPath }/productfindByCid.action?cid=${cid }&page=1" class="firstPage">&nbsp;</a>		
				<a href="${ pageContext.request.contextPath }/productfindByCid.action?cid=${cid }&page=${pageBean.page-1}" class="previousPage">&nbsp;</a>	
			</c:if>
			<c:forEach var="i" begin="1" end="${pageBean.totalPage }" step="1">
				...
				<c:if test="${pageBean.page!=i}">
					<a href="${ pageContext.request.contextPath }/productfindByCid.action?cid=${cid }&page=${i}">${i}</a>
				</c:if>
			</c:forEach>
			<c:if test="${pageBean.page != pageBean.totalPage }">
				<a class="nextPage" href="${ pageContext.request.contextPath }/productfindByCid.action?cid=${cid }&page=${pageBean.page+1}">&nbsp;</a>
				<a class="lastPage" href="${ pageContext.request.contextPath }/productfindByCid.action?cid=${cid }&page=${pageBean.totalPage}">&nbsp;</a>
			</c:if>
	</div>
			</form>
		</div>
